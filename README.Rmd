...
ratio_small  <- round(as.numeric(bm$small$median[2])  / as.numeric(bm$small$median[1]),  1)
ratio_medium <- round(as.numeric(bm$medium$median[2]) / as.numeric(bm$medium$median[1]), 1)
mem_small_pct <- round(
  (as.numeric(bm$small$mem_alloc[2]) - as.numeric(bm$small$mem_alloc[1])) /
    as.numeric(bm$small$mem_alloc[2]) * 100
)
...
On the included 6,691-respondent dataset raked on 10 variables, autumn's median runtime is `r ratio_small`× faster than anesrake and allocates `r mem_small_pct`% less memory:

```{r bm_small, echo = FALSE}
print(bm$small[, c("expression", "median", "mem_alloc")])
```

On a harder problem — the same dataset raked on 17 variables — autumn is `r ratio_medium`× faster and allocates substantially less memory:
...
- **Unbounded rake** (`max_weight = Inf`) is `r unb_vs_bounded`× faster than bounded rake on this dataset — the weight cap forces additional constraint-satisfaction work per cell per iteration. Use it when no cap is needed.
- **SQUAREM** (`accelerate = TRUE`) is `r if (squarem_vs_base >= 1) paste0(squarem_vs_base, "× *slower*") else paste0(round(1/squarem_vs_base, 1), "× *faster*")` on this well-conditioned bounded dataset. Both plain IPF and SQUAREM exit via the `pct` convergence criterion within 3–5 passes on this problem, so SQUAREM's iteration savings are minimal; its per-super-step overhead determines the net result. Use SQUAREM on poorly-conditioned problems where plain IPF requires thousands of iterations.
- **Unbounded calibrate** (`method = "calibrate", max_weight = Inf`) is `r cal_unb_vs_rake_unb`× slower than unbounded rake on this 10-variable problem. `calibrate`'s advantage emerges on severely imbalanced data where IPF requires hundreds of iterations; a single K×K Newton step costs more but converges in 10–20 steps versus 500+.
- **Bounded calibrate** (`method = "calibrate", max_weight = 50`) is `r if (cal_bnd_vs_rake_bnd < 1) paste0(round(1/cal_bnd_vs_rake_bnd, 1), "× *faster*") else paste0(cal_bnd_vs_rake_bnd, "× *slower*")` than bounded rake on this dataset. It uses logit distance (Deville & Särndal 1992), which enforces the cap by construction at every Newton step. `max_weight = 5` (the default) is infeasible for this dataset — the unconstrained raking max weight is ~43 — so `max_weight = 50` is used here; the timing gap partly reflects different cap regimes.
