...
squarem_speedup    <- round(m_med["rake_bounded"] / m_med["rake_bounded_squarem"], 1)
unb_vs_bounded     <- round(m_med["rake_bounded"] / m_med["rake_unbounded"],       1)
nr_unb_vs_rake_unb <- round(m_med["nr_unbounded"] / m_med["rake_unbounded"],       1)
```
...
- **Unbounded rake** (`max_weight = Inf`) is `r unb_vs_bounded`× faster than bounded rake on this well-conditioned problem — the weight cap forces additional constraint-satisfaction work per iteration. Use it when no cap is needed.
- **SQUAREM** (`accelerate = TRUE`) reduces bounded-rake runtime by `r squarem_speedup`× by cutting the iteration count. Gains are larger on slowly converging problems.
- **Unbounded NR** (`method = "nr", max_weight = Inf`) is `r nr_unb_vs_rake_unb`× slower than unbounded rake on this well-conditioned 10-variable problem. NR's advantage emerges on severely imbalanced data (10–20 NR steps versus hundreds of IPF iterations). Note that `accelerate` has no effect on `method = "nr"` — the bounded path hard-codes SQUAREM internally and the unbounded path is a direct K×K solve.
